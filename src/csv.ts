...
export function parseImportRows(csvText: string): { rows: NormalizedImportRow[]; errors: string[] } {
  ...
  // Colonnes « maths » en priorité, alias historiques (prénoms) conservés.
  const firstNameIndex = header.findIndex((value) => ['question', 'enonce', 'front', 'prenom', 'firstname', 'first_name'].includes(value));
  const hasHeader = firstNameIndex >= 0;
  const indexOf = (aliases: string[]) => header.findIndex((value) => aliases.includes(value));
  const lastNameIndex = indexOf(['reponse', 'answer', 'back', 'nom', 'lastname', 'last_name']);
  const contextIndex = indexOf(['indice', 'hint', 'contexte', 'context', 'role']);
